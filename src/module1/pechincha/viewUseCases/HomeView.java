...
public class HomeView extends ViewController {
	...
	private String getFeed(ActionDone ad) {
		...
		for (ManagerLeilao ml : array) {
			...
			m.setVariable("tempo", formatarHora(ml.getTempoCorrente()));
	...
	private String formatarHora(int seg) {
		float segundos = seg;

		float horas = (float) (segundos / 3600.0f);
		float minutos = (float) (segundos % 3600.0f);
		minutos = (float) (minutos / 60);

		int horaInt = (int) horas;
		int minutoInt = (int) minutos;
